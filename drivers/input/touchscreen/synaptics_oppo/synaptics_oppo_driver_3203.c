...
#ifdef VENDOR_EDIT
/*huqiao@EXP.BasicDrv, 2015/05/19  Add for 15069 QHD lcd START */
	  	if(GET_MODEM_VERSION()==103)
	  		{
				return sprintf(buf,
				  ...
				  "\n",90,button_map[2],button_map[0],button_map[1],270,button_map[2],button_map[0],button_map[1],450,button_map[2],button_map[0],button_map[1]);
...
/*huqiao@EXP.BasicDrv, 2015/05/19  Add for 15069 QHD lcd START */		
			  ...
			  return sprintf(buf,
				...
				":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":%d:%d:%d:%d"
				":" __stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":%d:%d:%d:%d"
				"\n",134,button_map[2],button_map[0],button_map[1],356,button_map[2],button_map[0],button_map[1],590,button_map[2],button_map[0],button_map[1]);
	  } else if(is_project(OPPO_15037)){
		  return sprintf(buf,
				__stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":%d:%d:%d:%d"
			":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":%d:%d:%d:%d"
			":" __stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":%d:%d:%d:%d"
			"\n",100,button_map[2],button_map[0],button_map[1],267,button_map[2],button_map[0],button_map[1],442,button_map[2],button_map[0],button_map[1]);
	 }else {
		 return sprintf(buf,
				__stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":%d:%d:%d:%d"
			":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":%d:%d:%d:%d"
...
static int synaptics_parse_dts(struct device *dev, struct synaptics_ts_data *ts)
{
...
#ifdef VENDOR_EDIT
	/*huqiao@EXP.BasicDrv, 2015/03/20  Add for 15069 QHD lcd START */
	/*Lile@EXP.BasicDrv 2015-05-17, Add for 15070 and 15091 LCD crash in LK, search key words MSM_15069 */
	if(is_project(OPPO_15009)&&GET_MODEM_VERSION() == 103)//GPIO 106 17 16 
		{
			rc = of_property_read_u32_array(np, "synaptics,button-map-extra", button_map, 3);
			if(rc){
	...
	/*huqiao@EXP.BasicDrv, 2015/03/20  Add for 15069 QHD lcd END */
#endif
	...
	/*huqiao@EXP.BasicDrv, 2015/03/20  Add for 15069 QHD lcd START */
	/*Lile@EXP.BasicDrv 2015-05-17, Add for 15070 and 15091 LCD crash in LK, search key words MSM_15069 */
	if(is_project(OPPO_15009)&&GET_MODEM_VERSION() == 103)
		{
			rc = of_property_read_u32_array(np, "synaptics,display-coords-extra", temp_array, 2);
		if(rc){
	...
	if( (tp_dev == TP_G2Y) || (tp_dev == TP_TPK) ) {
       ...
       if (is_project(OPPO_15035) || is_project(OPPO_16000)){
        	if(tp_dev == TP_OFILM){
        		...
        		strcpy(ts->fw_name,"tp/15035/15035_Firmware_Oflim.img");
        		strcpy(ts->test_limit_name,"tp/15035/15035_Limit_Oflim.img");
        	} else {
        		sprintf(ts->manu_name, "TP_TRULY"); 
        		strcpy(ts->fw_name,"tp/15035/15035_Firmware_Truly.img");
        		strcpy(ts->test_limit_name,"tp/15035/15035_Limit_Truly.img");
        	}        
       }
       else{
        	if(tp_dev == TP_OFILM){
        		sprintf(ts->manu_name, "TP_OFILM");	
        		strcpy(ts->fw_name,"tp/15009/15009_Firmware_Oflim.img");
        		strcpy(ts->test_limit_name,"tp/15009/15009_Limit_Oflim.img");
        	} else {
        		sprintf(ts->manu_name, "TP_TRULY"); 
        		strcpy(ts->fw_name,"tp/15009/15009_Firmware_Truly.img");
        		strcpy(ts->test_limit_name,"tp/15009/15009_Limit_Truly.img");
        	}
       }
